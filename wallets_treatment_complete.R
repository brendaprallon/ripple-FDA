...
wallets_treatment = function(dt, btc_price, lifespan){
    # filtering out wallets with < 500
    ...
    # # increments column
    # dt[, lag := shift(value, n=1, fill=NA, type="lag"), by = address]
    # dt[!is.na(lag), increments := value - lag]
    # dt[is.na(lag), increments := value]
    # 
    # # Credit and debt
    # dt[increments >= 0, increment_type :=  "credit"]
    # dt[increments < 0, increment_type := "debt"]
    ...
    # dt = dt[,`:=` (value = sum(value), lag = sum(lag), increments = sum(increments)), by = .(address, increment_type, date)]
    # dt = unique(dt)
    
    # creating new count variable
    dt[, count := .N, by = address]
    
    # NEW: changing value name to increment
...
# Table start dates per category
table_ltx_date_cat = function(dt){
...
balanced_sample = function(dt, min_obs, plt_sample_size){
    ...
    min_obs_cat = min(cat_dt$count) # uncomment 
    # sampling from each category
    balanced_sample_addr = list()
    plt_sample_addr = list()
    for (cat in cat_dt$category){
        cat_count = cat_dt[category == cat, count] # comment
        if (cat_count < min_obs_cat){ # comment
            balanced_sample_addr[[cat]] = sample(unique(dt[category == cat, address]), cat_count) # comment
        }else{ # comment
    ...
    # save lists
    saveRDS(balanced_sample_addr, paste0("data/treated/balanced samples/balanced_sample_addr_", min_obs, "obs.rds"))
    saveRDS(plt_sample_addr, paste0("data/treated/balanced samples/plt_sample_addr_", min_obs, "obs.rds"))
    # save dt
...
# Appending was only necessary to be run once; following lines are commented because of that
#files = c(paste0("data/raw/wallets_series_sample_", 1:83, ".rds"), "data/raw/wallets_series_sample.rds")
#dt = rbindlist(lapply(files, readRDS))
#saveRDS(dt, "data/raw/wallets_time_series_appended.rds")
#dt = readRDS("data/raw/wallets_time_series_appended.rds")
...
# Treating the data: Using lifespan of 2000
# dt = wallets_treatment(copy(dt), copy(btc_price), lifespan = 2000)
# table_ltx_count_cat(copy(dt))
# table_ltx_date_cat(copy(dt))
# ## Filtering Min Obs
# set.seed(123)
# # 10 obs
# filter_min_obs(copy(dt), 10, 2000)
# # 20 obs
# filter_min_obs(copy(dt), 20, 2000)
