...
create_partition = function(min_obs, p_test, n_kfold_train, binary = FALSE){
    ...
    # dt = dt[address %in% unlist(addr_samples)]
    # dt[category %in% c("Broker", "CEX", "DEX"), category := "Exchange"]
    # dt[category %in% c("Mixer Service", "Payment System", "Faucet"), category := "Service"]
    #dt[category == "Faucet", category := "Mining"]
    ###
    if (binary == TRUE){
